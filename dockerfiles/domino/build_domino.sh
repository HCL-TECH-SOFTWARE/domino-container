...
docker_build ()
{
  ...
  docker build --no-cache --label "DominoDocker.description"="$DOCKER_DESCRIPTION" --label "DominoDocker.version"="$DOCKER_IMAGE_VERSION" --label "DominoDocker.buildtime"="$BUILDTIME" -t $DOCKER_IMAGE $DOCKER_TAG_LATEST_CMD -f $DOCKER_FILE $BUILD_ARG_DOWNLOAD_FROM $BUILD_ARG_PROD_NAME $BUILD_ARG_PROD_VER $BUILD_ARG_PROD_FP $BUILD_ARG_PROD_HF .
